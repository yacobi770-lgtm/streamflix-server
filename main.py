...
def send_email(to: str, subject: str, html: str):
    ...
    try:
        ...
        msg["Subject"] = subject
        msg["From"] = f"StreamFlix <{SMTP_USER}>"
        msg["To"] = to
...
async def register(request: Request, background_tasks: BackgroundTasks):
    ...
    try:
        async with pool.acquire() as conn:
            ...
            background_tasks.add_task(send_email, email, "אמת את האימייל שלך — StreamFlix", f"""
                <div dir="rtl" style="font-family:sans-serif;max-width:400px;margin:auto;padding:2rem">
                <h2 style="color:#4F8EFF">StreamFlix 🎬</h2>
                <p>קוד האימות שלך:</p>
    ...
    if pool:
        async with pool.acquire() as conn:
            ...
            if user:
                ...
                background_tasks.add_task(send_email, email, "איפוס סיסמה — StreamFlix", f"""
                    <div dir="rtl" style="font-family:sans-serif;max-width:400px;margin:auto;padding:2rem">
                    <h2 style="color:#4F8EFF">StreamFlix 🎬</h2>
...
async def get_ratings(item_id: str):
    ...
    if pool:
        async with pool.acquire() as conn:
            ...
            return JSONResponse({"ratings": [dict(r) for r in rows], "average": float(avg) if avg else None})
