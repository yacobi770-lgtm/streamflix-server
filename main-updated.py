...
async def get_popular(page: int = 1):
    """Get popular movies - used by app.html"""
    try:
        async with httpx.AsyncClient() as client:
            url = f"https://api.themoviedb.org/3/movie/popular?api_key={TMDB_API_KEY}&language=he-IL&page={page}"
...
async def search_movies(query: str, page: int = 1):
    """Search for movies and shows - used by app.html"""
    try:
        async with httpx.AsyncClient() as client:
            url = f"https://api.themoviedb.org/3/search/multi?api_key={TMDB_API_KEY}&query={query}&language=he-IL&page={page}"
            response = await client.get(url, timeout=10)
            return JSONResponse(status_code=200, content=response.json())
    except Exception as e:
        return JSONResponse(status_code=500, content={"detail": str(e)})
